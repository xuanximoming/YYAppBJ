...
public class MRpinggudescActivity extends Activity implements OnClickListener {
	...
	private void init() {
		...
		activity_title_name.setText("评估详情");
				...
				case 101:
					...
					if (data == null) {
						if (Statics.internetcode != -1) {
							MyUntils.myToast(MRpinggudescActivity.this,
									Statics.internetstate[Statics.internetcode]);
						}
					} else if (data.size() > 0) {
						System.out.println(data.toString());
						adapter = new JianChaAdapter(data,
								MRpinggudescActivity.this);
						listview.setAdapter(adapter);
					} else {
//						MyUntils.myToast(MRpinggudescActivity.this, "获取信息失败");
					}
					break;
