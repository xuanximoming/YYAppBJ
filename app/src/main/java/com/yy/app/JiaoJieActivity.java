...
import com.yy.adapter.JiaoJieSelectDateAdapter;
...
public class JiaoJieActivity extends Activity implements OnClickListener {
	...
	private void init() {
		...
		gridview.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				// TODO Auto-generated method stub
				Intent intent = new Intent(JiaoJieActivity.this,
						JiaoJieDesActivity.class);
				intent.putExtra("date", data.get(position).getJIAOBAN_DATE());
				...
				case 101:
					...
					if (data == null) {
						if (Statics.internetcode != -1) {
							MyUntils.myToast(JiaoJieActivity.this,
									Statics.internetstate[Statics.internetcode]);
						}else{
							MyUntils.myToast(JiaoJieActivity.this,"此病人暂无交接班信息");
						}
					} else if (data.size() > 0) {
                        System.out.println("data="+data+data.size());
						System.out.println(data.toString());
						adapter = new JiaoJieSelectDateAdapter(data, JiaoJieActivity.this);
	...
	void downlaoddata() {
		...
		ThreadPoolManager.getInstance().addTask(new Runnable() {
			public void run() {
				...
				data = MyUntils.JiaoJieBanlist(patient_id, zid);
	...
	public void onClick(View v) {
		...
		case R.id.activity_title_ok:
			startActivity(new Intent(this, AddJiaoJieActivity.class));
			break;
