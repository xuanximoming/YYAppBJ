...
public class MRPGSelectDateActivity extends Activity implements OnClickListener {
	...
	private void init() {
		...
		gridview.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				// TODO Auto-generated method stub
				Intent intent = new Intent(MRPGSelectDateActivity.this,
						MRPingguActivity.class);
				...
				case 101:
					...
					if (data == null) {
						if (Statics.internetcode != -1) {
							MyUntils.myToast(MRPGSelectDateActivity.this,
									Statics.internetstate[Statics.internetcode]);
						}else{
							MyUntils.myToast(MRPGSelectDateActivity.this,"此病人暂无每日评估历史信息");
						}
					} else if(data.size()>0) {
						System.out.println(data.toString());
						adapter = new HuLiSelectDateAdapter(data,
								MRPGSelectDateActivity.this);
	...
	void downlaoddata() {
		...
		ThreadPoolManager.getInstance().addTask(new Runnable() {
			public void run() {
				...
				data = MyUntils.getPingGuList(patient_id, zid);
